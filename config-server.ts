 ...
 * 
 * This is a simple HTTP server that handles saving configuration to .env file
...
// Get directory path (works in both CommonJS and ESM)
...
const ENV_FILE = path.join(__dirname_local, '.env');
...
function writeConfig(config: Record<string, string>): boolean {
    try {
...
function restartServer(): Promise<boolean> {
    return new Promise((resolve) => {
        console.log('Restarting y-router container...');
        exec('docker restart y-router-y-router-1', { cwd: __dirname_local }, (error: Error | null, stdout: string, stderr: string) => {
            if (error) {
